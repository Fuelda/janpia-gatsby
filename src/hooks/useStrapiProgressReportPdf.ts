...
const useStrapiProgressReportPdf = (slug: string, endpoint: string) => {
    ...
    const fetchStrapiPdf = async () => {
      ...
      try {
        const fdoResponse = await fetch(
          `${strapiApiURL}api/${endpoint}?populate[data][fields][0]=url&populate[progress_round][fields][0]=code&filters[$and][0][biz_cd_fund_distr][$eq]=${slug}&filters[$and][1][business_org_type][$eq]=F`
        );
        const adoResponse = await fetch(
          `${strapiApiURL}api/${endpoint}?populate[data][fields][0]=url&populate[progress_round][fields][0]=code&filters[$and][0][biz_cd_executive][$eq]=${slug}&filters[$and][1][business_org_type][$eq]=A`
        );
        ...
        if (fdoData.data && fdoData.data.length > 0) {
          dataArray = fdoData.data.map((item: any) => {
            return {
              ...
              round: item.attributes.progress_round.data.attributes.code,
...
export default useStrapiProgressReportPdf;
